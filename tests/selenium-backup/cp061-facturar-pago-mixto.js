...
async function cp061_facturar_pago_mixto() {
  ...
  try {
    ...
    `);
    if (!added) {
      console.log('❌ CP-061 FAILED: No se encontró el producto de prueba');
      process.exit(1);
    }
    ...
    if (customerSelected !== '12735') {
      console.log('❌ CP-061 FAILED: No se pudo asociar el cliente de prueba (id 12735) a la factura');
      process.exit(1);
    }
    ...
    // Habilitar Tarjeta además de Efectivo (pago mixto) y distribuir el monto
    // total entre ambos métodos. El campo payment_cash_total viene
    // pre-cargado con el total a pagar cuando solo Efectivo está activo; se
    // usa ese valor como base para la distribución 50/50.
    ...
    `);
    if (!(totalToPay > 0)) {
      console.log('❌ CP-061 FAILED: No se pudo determinar el monto total a pagar');
      process.exit(1);
    }
    ...
    await driver.executeScript(`
      ...
      setVal('payment_cash_total', ${half});
      setVal('payment_credit_card_total', ${remainder});
    `);
    await driver.sleep(500);
    ...
    if (cartEmpty) {
      ...
      console.log('❌ CP-061 FAILED: La factura con pago mixto no se confirmó (el producto sigue en el carrito)');
      process.exit(1);
    }
  } catch (error) {

...
async function cp058_facturar_producto_gravado() {
  ...
  try {
    ...
    // Buscar y agregar un producto gravado (con IVA): "AAA-Multímetro
    // Automotriz Digital", ya confirmado con IVA > 0 en el panel de totales.
    ...
    `);
    if (!added) {
      console.log('❌ CP-058 FAILED: No se encontró el producto gravado de prueba');
      process.exit(1);
    }
    ...
    // Verificar que el IVA se calculó (> 0) en el resumen
    ...
    if (!(ivaValue > 0)) {
      console.log('❌ CP-058 FAILED: El IVA calculado no es mayor a cero para el producto gravado (₡' + ivaValue + ')');
      process.exit(1);
    }

    // Asociar un cliente real y registrado (id 12735) directamente vía
    // selectCustomerToPos(): el buscador visual #search_pos_customer no
    // devuelve resultados (ver CP-034) y el flujo de "cliente rápido" resultó
    // intermitente con la nueva validación de la versión actual del sistema
    // (a veces no completaba el registro a tiempo para la factura).
    ...
    if (customerSelected !== '12735') {
      console.log('❌ CP-058 FAILED: No se pudo asociar el cliente de prueba (id 12735) a la factura');
      process.exit(1);
    }
    // selectCustomerToPos() dispara llamadas AJAX internas (p. ej. lista de
    // precios del cliente); se espera un momento a que asienten antes de
    // abrir el modal de pago, para evitar una condición de carrera.
    ...
    if (cartEmpty) {
      ...
      console.log('❌ CP-058 FAILED: La factura no se confirmó (el producto sigue en el carrito)');
      process.exit(1);
    }
  } catch (error) {

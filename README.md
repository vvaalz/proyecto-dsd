...
| 07-citas | 01-crear-editar-citas | CP-204 – CP-207 | `tests-playwright/07-citas/01-crear-editar-citas/` |
...
"07-citas/01-crear-editar-citas" (módulo "Citas" del menú lateral, `/reservation/reservation`, calendario FullCalendar con vistas Mes/Semana/Día/Agenda) — módulo nuevo, no automatizado antes. 4 CPs (CP-204–CP-207) cubriendo el ciclo real de una cita: CP-204 crea una cita básica (asunto, fecha/hora, cliente) y confirma el guardado por red; CP-205 abre el detalle de una cita desde la vista Agenda y confirma sus acciones reales (Eliminar/WhatsApp/Email/Editar) además de que "Total General" se mantiene en ₡0.00 sin servicios/productos agregados; CP-206 edita una cita existente (asunto y hora) y confirma que persiste sobre el MISMO id, no uno nuevo; CP-207 cancela una cita y confirma que el servidor la marca `is_active=0` (soft-delete, no eliminación real — sigue en el feed del mes). **Se detuvo deliberadamente antes de dos puntos que cruzan con el hallazgo crítico de montos corruptos** (`CLAUDE_CONTEXT.md` sección 22): la sección "Servicios y Productos" del formulario de cita (calcula Total Servicios/Total Productos/Total General igual que el carrito del POS) y el botón "Convertir a orden" (crea una orden de taller real a partir de la cita) — ninguno de los 2 se ejerció, pendientes de decisión del usuario. Ver hallazgos técnicos completos (buscador de cliente NO es autocompletado, endpoints reales de guardar/ver/eliminar, por qué el calendario no sirve para verificar por texto) en `CLAUDE_CONTEXT.md` sección 28.

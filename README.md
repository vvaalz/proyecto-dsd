...
**🔴 Módulo "Ventas"** (ítem de acordeón del menú lateral, 6 sub-ítems: Histórico de Ventas, Abono Cuentas por Cobrar, Lista de Cobros, Historial Mov. de Caja, Devoluciones, Nota de crédito) — **explorado en vivo, sin ningún CP creado**. Confirmado que es un módulo de reportes/gestión de ventas ya realizadas (distinto de "Facturar"/POS). **Hallazgo (2026-08-01)**: el bug crítico de montos corruptos (`CLAUDE_CONTEXT.md` sección 22) ya no se limita al carrito en vivo — se confirmó en registros PERSISTIDOS reales: 8 facturas consecutivas en Histórico de Ventas con el mismo monto corrupto exacto ya visto en CP-200, notas de crédito individuales corruptas, y el panel resumen agregado de "Nota de crédito" mostrando ₡57 mil millones de "Monto total" acumulado. Por instrucción explícita, no se escribió ningún CP de cobertura para este módulo — se documentó el hallazgo y quedó pendiente de decisión del usuario. Ver detalle completo y evidencia en `CLAUDE_CONTEXT.md` sección 27 y `PLAN-PROYECTO-FINAL.md` punto 12.

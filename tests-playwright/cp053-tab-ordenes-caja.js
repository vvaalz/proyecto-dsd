...
async function cp053_tab_ordenes_caja() {
  ...
  try {
    ...
    if (!(await page.evaluate(() => !!document.getElementById('btn_cashier_option')))) {
      throw new Error('No se encontró el tab "(F2) Órdenes de caja"');
    }
    const t1 = Date.now();
    await page.evaluate(() => document.getElementById('btn_cashier_option').click());
    await page.waitForTimeout(2500);
    console.log('⏱ Cargar tab Órdenes de caja: ' + (Date.now() - t1) + 'ms');
    ...
    if (loaded) {
      ...
      throw new Error('El tab "(F2) Órdenes de caja" no mostró contenido reconocible');
    }
  } catch (error) {
    const dir = path.join(__dirname, '..', 'reports', 'screenshots');
    fs.mkdirSync(dir, { recursive: true });
    try { await page.screenshot({ path: path.join(dir, 'cp053-fallo-' + Date.now() + '.png'), timeout: 5000 }); } catch {}

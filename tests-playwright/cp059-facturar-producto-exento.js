...
async function cp059_facturar_producto_exento() {
  ...
  try {
    ...
    if (!added) throw new Error('No se encontró el producto exento de prueba');

    await page.waitForSelector('#tb_table_buy_list', { timeout: 20000 });
    await page.waitForFunction(() => /aaa-bombillos/i.test(document.getElementById('tb_table_buy_list').textContent), { timeout: 10000 });
    ...
    if (!(ivaValue === 0)) throw new Error('El IVA calculado no es cero para el producto exento (₡' + ivaValue + ')');
    ...
    const cartEmpty = await confirmSweetAlerts(page, 'aaa-bombillos');
    ...
    try { await page.screenshot({ path: path.join(dir, 'cp059-fallo-' + Date.now() + '.png'), timeout: 5000 }); } catch {}

...
const BTN_ID = 'dashboard_button_setting_18';
const CONTENT_ID = 'dashboard_content_settings_18';
const CAMPO_ID = 'date_external_purchases_checkbox';
...
async function cp152_compras_externas() {
  console.log('🔄 Ejecutando CP-152: Panel de Control — Compras externas...');
  ...
  try {
    ...
    if (!seccionAbierta) { await screenshotOnFail(page, 'cp152-fail-seccion-no-abre'); throw new Error('La sección "Compras externas" no se pudo expandir'); }

    valorOriginal = await page.evaluate((id) => document.getElementById(id)?.checked, CAMPO_ID);
    console.log('📋 Valor original de "¿Cambiar fecha a dd/mm/aaaa?":', valorOriginal);
    if (valorOriginal === null || valorOriginal === undefined) { await screenshotOnFail(page, 'cp152-fail-campo-no-encontrado'); throw new Error('No se encontró el campo #' + CAMPO_ID); }
    ...
    console.log('✅ CP-152 PASSED | campo: #' + CAMPO_ID + ' | ' + valorOriginal + ' → ' + !valorOriginal + ' → ' + valorOriginal + ' (restaurado) | validaciones: 2/2 | tiempo: ' + tiempoTotal + 'ms');
    registrarResultado({ cp: 'CP-152', modulo: moduloDesdeRuta(__dirname), estado: 'pass', tiempoMs: tiempoTotal });

  } catch (error) {
    await screenshotOnFail(page, 'cp152-fail');
    console.log('❌ CP-152 FAILED: ' + error.message);
    ...
    registrarResultado({ cp: 'CP-152', modulo: moduloDesdeRuta(__dirname), estado: 'fail', tiempoMs: Date.now() - tiempoInicioCP });
    process.exit(1);
  } finally {
    await browser.close();
  }
}
cp152_compras_externas();

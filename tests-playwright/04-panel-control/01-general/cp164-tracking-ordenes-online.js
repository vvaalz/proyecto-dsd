...
const BTN_ID = 'dashboard_button_setting_11';
const CONTENT_ID = 'dashboard_content_settings_11';
// La sección tiene además un 4to checkbox, #enable_hide_fields_online_repair_order, que
// revela una tabla anidada de ~23 checkboxes SIN id propio (respaldados por un solo campo
// hidden JSON, #online_repair_order_hidden_fields_json) para ocultar campos específicos del
// PDF/vista online de la orden. Es una sub-funcionalidad distinta (array JSON, no booleano
// simple) — no cubierta por este CP, candidata a un CP propio más adelante si hace falta.
const CAMPO_ID = 'show_prices_totals_customer_order_tracking_checkbox';
...
async function cp164_tracking_ordenes_online() {
  console.log('🔄 Ejecutando CP-164: Panel de Control — Tracking de órdenes online para clientes...');
  ...
  try {
    ...
    if (!seccionAbierta) { await screenshotOnFail(page, 'cp164-fail-seccion-no-abre'); throw new Error('La sección "Tracking de órdenes online para clientes" no se pudo expandir'); }

    valorOriginal = await page.evaluate((id) => document.getElementById(id)?.checked, CAMPO_ID);
    console.log('📋 Valor original de "¿Mostrar precios y totales en el tracking de órdenes online?":', valorOriginal);
    if (valorOriginal === null || valorOriginal === undefined) { await screenshotOnFail(page, 'cp164-fail-campo-no-encontrado'); throw new Error('No se encontró el campo #' + CAMPO_ID); }
    ...
    console.log('✅ CP-164 PASSED | campo: #' + CAMPO_ID + ' | ' + valorOriginal + ' → ' + !valorOriginal + ' → ' + valorOriginal + ' (restaurado) | validaciones: 2/2 | tiempo: ' + tiempoTotal + 'ms');
    registrarResultado({ cp: 'CP-164', modulo: moduloDesdeRuta(__dirname), estado: 'pass', tiempoMs: tiempoTotal });

  } catch (error) {
    await screenshotOnFail(page, 'cp164-fail');
    console.log('❌ CP-164 FAILED: ' + error.message);
    ...
    registrarResultado({ cp: 'CP-164', modulo: moduloDesdeRuta(__dirname), estado: 'fail', tiempoMs: Date.now() - tiempoInicioCP });
    process.exit(1);
  } finally {
    await browser.close();
  }
}
cp164_tracking_ordenes_online();

...
const BTN_ID = 'dashboard_button_setting_4';
const CONTENT_ID = 'dashboard_content_settings_4';
const CAMPO_ID = 'generate_automatic_product_code';
...
async function cp157_configuracion_de_inventario() {
  console.log('🔄 Ejecutando CP-157: Panel de Control — Configuración de inventario...');
  ...
  try {
    ...
    if (!seccionAbierta) { await screenshotOnFail(page, 'cp157-fail-seccion-no-abre'); throw new Error('La sección "Configuración de inventario" no se pudo expandir'); }

    valorOriginal = await page.evaluate((id) => document.getElementById(id)?.checked, CAMPO_ID);
    console.log('📋 Valor original de "Generar código interno automático":', valorOriginal);
    if (valorOriginal === null || valorOriginal === undefined) { await screenshotOnFail(page, 'cp157-fail-campo-no-encontrado'); throw new Error('No se encontró el campo #' + CAMPO_ID); }
    ...
    console.log('✅ CP-157 PASSED | campo: #' + CAMPO_ID + ' | ' + valorOriginal + ' → ' + !valorOriginal + ' → ' + valorOriginal + ' (restaurado) | validaciones: 2/2 | tiempo: ' + tiempoTotal + 'ms');
    registrarResultado({ cp: 'CP-157', modulo: moduloDesdeRuta(__dirname), estado: 'pass', tiempoMs: tiempoTotal });

  } catch (error) {
    await screenshotOnFail(page, 'cp157-fail');
    console.log('❌ CP-157 FAILED: ' + error.message);
    ...
    registrarResultado({ cp: 'CP-157', modulo: moduloDesdeRuta(__dirname), estado: 'fail', tiempoMs: Date.now() - tiempoInicioCP });
    process.exit(1);
  } finally {
    await browser.close();
  }
}
cp157_configuracion_de_inventario();

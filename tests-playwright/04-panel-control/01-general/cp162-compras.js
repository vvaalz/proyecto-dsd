...
const BTN_ID = 'dashboard_button_setting_17';
const CONTENT_ID = 'dashboard_content_settings_17';
const CAMPO_ID = 'show_image_purchase_proform_checkbox';
...
async function cp162_compras() {
  console.log('🔄 Ejecutando CP-162: Panel de Control — Compras...');
  ...
  try {
    ...
    if (!seccionAbierta) { await screenshotOnFail(page, 'cp162-fail-seccion-no-abre'); throw new Error('La sección "Compras" no se pudo expandir'); }

    valorOriginal = await page.evaluate((id) => document.getElementById(id)?.checked, CAMPO_ID);
    console.log('📋 Valor original de "Adjuntar Imágenes a Órdenes de Compra":', valorOriginal);
    if (valorOriginal === null || valorOriginal === undefined) { await screenshotOnFail(page, 'cp162-fail-campo-no-encontrado'); throw new Error('No se encontró el campo #' + CAMPO_ID); }
    ...
    console.log('✅ CP-162 PASSED | campo: #' + CAMPO_ID + ' | ' + valorOriginal + ' → ' + !valorOriginal + ' → ' + valorOriginal + ' (restaurado) | validaciones: 2/2 | tiempo: ' + tiempoTotal + 'ms');
    registrarResultado({ cp: 'CP-162', modulo: moduloDesdeRuta(__dirname), estado: 'pass', tiempoMs: tiempoTotal });

  } catch (error) {
    await screenshotOnFail(page, 'cp162-fail');
    console.log('❌ CP-162 FAILED: ' + error.message);
    ...
    registrarResultado({ cp: 'CP-162', modulo: moduloDesdeRuta(__dirname), estado: 'fail', tiempoMs: Date.now() - tiempoInicioCP });
    process.exit(1);
  } finally {
    await browser.close();
  }

...
const BTN_ID = 'dashboard_button_setting_19';
const CONTENT_ID = 'dashboard_content_settings_19';
const CAMPO_ID = 'points_by_company_checkbox';
...
async function togglearCheckbox(page, id, valor) {
  ...
  // se envía al guardar — hace falta disparar 'click' además de 'change'.
...
async function cp153_fidelidad_de_clientes() {
  console.log('🔄 Ejecutando CP-153: Panel de Control — Fidelidad de clientes...');
  ...
  try {
    ...
    if (!seccionAbierta) { await screenshotOnFail(page, 'cp153-fail-seccion-no-abre'); throw new Error('La sección "Fidelidad de clientes" no se pudo expandir'); }

    valorOriginal = await page.evaluate((id) => document.getElementById(id)?.checked, CAMPO_ID);
    console.log('📋 Valor original de "¿Acumular puntos individual por compañía?":', valorOriginal);
    if (valorOriginal === null || valorOriginal === undefined) { await screenshotOnFail(page, 'cp153-fail-campo-no-encontrado'); throw new Error('No se encontró el campo #' + CAMPO_ID); }
    ...
    console.log('✅ CP-153 PASSED | campo: #' + CAMPO_ID + ' | ' + valorOriginal + ' → ' + !valorOriginal + ' → ' + valorOriginal + ' (restaurado) | validaciones: 2/2 | tiempo: ' + tiempoTotal + 'ms');
    registrarResultado({ cp: 'CP-153', modulo: moduloDesdeRuta(__dirname), estado: 'pass', tiempoMs: tiempoTotal });

  } catch (error) {
    await screenshotOnFail(page, 'cp153-fail');
    console.log('❌ CP-153 FAILED: ' + error.message);
    ...
    registrarResultado({ cp: 'CP-153', modulo: moduloDesdeRuta(__dirname), estado: 'fail', tiempoMs: Date.now() - tiempoInicioCP });
    process.exit(1);
  } finally {
    await browser.close();
  }
}
cp153_fidelidad_de_clientes();

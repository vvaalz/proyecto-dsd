...
async function cp043_cambio_moneda_pos() {
  ...
  try {
    ...
    if (!currencyBtnExists) throw new Error('No se encontró el selector de moneda (CRC) en el encabezado');
    ...
    const menuText = await page.evaluate(() => {
      ...
      const menu = Array.from(document.querySelectorAll('.mdl-menu')).filter(isVis).find(m => /d[oó]lar|euro|colón/i.test(m.textContent||''));
    ...
    if (missing.length === 0) {
      ...
      throw new Error('Faltan opciones de moneda: ' + JSON.stringify(missing));
    }
  } catch (error) {
    const dir = path.join(__dirname, '..', '..', '..', 'reports', 'screenshots');
    fs.mkdirSync(dir, { recursive: true });
    try { await page.screenshot({ path: path.join(dir, 'cp043-fallo-' + Date.now() + '.png'), timeout: 5000 }); } catch {}

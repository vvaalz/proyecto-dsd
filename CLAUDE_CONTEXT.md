...
### Acciones sobre una orden de ruteo YA EXISTENTE (tarjeta del tablero) — para CP-141
...
- Botón `more_vert` (`button[data-toggle="dropdown"]` dentro de `.product_dropdown_options`, id dinámico tipo `dLabel<N>` — no confiar en el id, buscar por el ícono `more_vert` dentro de la tarjeta) → despliega `ul.dropdown-menu` con 5 acciones:
  ...
  - "Editar órden" → `onclick="show_create_routing_order_modal(<ID>)"` — **reutiliza el mismo modal `#dialog_add_routing_order`** de la creación, mismos IDs de campos, pero pre-poblado (edición in-place del mismo formulario, no un modal distinto)
  - "Marcar como EN CAMINO" → `onclick="change_routing_order_status(<ID>, 2)"`
  - "Marcar como ENTREGADO" → `onclick="change_routing_order_status(<ID>, 3)"`
  - "Eliminar órden" → `onclick="show_confirm_delete_routing_order(<ID>)"` — **acción destructiva**, usar solo sobre órdenes creadas por el propio CP
  - (estados numéricos inferidos: 1=Pendiente por defecto, 2=En Camino, 3=Entregado — coincide con los filtros `filter_routing_order_btn_pending/in_route/delivered`)

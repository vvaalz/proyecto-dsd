...
15. Consecutivos Comprobantes — **0 campos, confirmado vacía para esta compañía (2026-07-08)**: el contenido real es solo un aviso "¡Atención!" y un contenedor `#display_honduras_consecutive_detail_content` que queda vacío — por el nombre del id, es una sección pensada para mostrar consecutivos específicos de Honduras vía AJAX condicional, que no aplica a esta compañía (Costa Rica, `country_id=47`). No amerita CP propio: no hay nada que probar (no es una limitación ni un bug, es contenido condicional por país que no corresponde renderizar aquí).
...
- **No asumir que una sección con "0 campos" es un bug**: confirmado en la sección 15 "Consecutivos Comprobantes" — a veces es contenido condicional por país/config que legítimamente no renderiza nada para esta compañía (ver detalle en la lista de secciones más abajo). Verificar el HTML real antes de escribir un CP o de reportarlo como hallazgo.

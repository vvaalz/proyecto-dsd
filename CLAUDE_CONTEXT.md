...
### Reconfirmación (2026-07-26, CP-200) — el hallazgo sigue activo, y es MÁS grave de lo documentado: corrupción parece ser de estado global, no por producto
Al cubrir el gap "combo multi-tipo de producto en un carrito" (producto normal + rápido + fraccionado + servicio en una sola venta, nunca antes probado junto), se diseñó `CP-200` para agregar los productos de forma INCREMENTAL, comparando tras cada uno el total real del carrito contra la SUMA de precios reales (extraídos del propio atributo `onclick="add_to_table(id,nombre,PRECIO,...)"` de cada tarjeta — no del texto visible, que la sección 22 ya había confirmado que puede mostrar un número corrupto en el listado aunque el `onclick` lleve el precio correcto).

- Carrito limpio confirmado (0 filas) → se agregó **AAA-Bombillos / luces halógenas** (precio real extraído del `onclick`: **₡1,000.00**, único producto en el carrito) → el total del carrito mostró **₡206,515,713.60** — el CP se detuvo de inmediato en este primer producto, tal como indicaba la instrucción del usuario, sin agregar el producto rápido, el fraccionado ni el servicio, y sin intentar ninguna otra combinación.
- **Hallazgo nuevo, más amplio que lo documentado hasta ahora**: en el screenshot de evidencia (`cp200-HALLAZGO-monto-corrupto-*.png`), **absolutamente todas** las tarjetas de producto visibles en el catálogo —incluyendo llantas, tequila, gato hidráulico, destornilladores, etc., productos que ni siquiera se tocaron— pasaron a mostrar el mismo número corrupto `₡206,515,713.60` que el total del carrito, apenas se agregó UN producto. Esto sugiere que la corrupción no es un cálculo erróneo aislado por producto/línea, sino una variable de estado global (ej. "último precio" o similar) que se usa incorrectamente para renderizar el precio de CADA tarjeta del catálogo, no solo el total del carrito.
- También se observó en la fila del carrito un dato adicional no documentado antes: un tooltip **"Utilidad: (900%) / ₡185864142.24"** junto al ícono de editar de la línea — otro número absurdo, distinto del total, que podría ser una pista adicional para el equipo de desarrollo sobre dónde se origina el cálculo corrupto (posible campo de "utilidad %" o "costo" mal referenciado).
- **Conclusión**: el hallazgo crítico de la sección 22 sigue activo sin cambios el 2026-07-26 (7 días después de la última reconfirmación, 2026-07-22) y se confirma que afecta el escenario de carrito combinado exactamente como se sospechaba — de hecho basta UN solo producto para corromper la vista completa del catálogo. **No se creó ningún CP de cobertura "feliz" para el combo multi-tipo** (normal+rápido+fraccionado+servicio) — `CP-200` documenta el hallazgo (registrado como ⚠️, no como fallo, ya que detectar y detener correctamente ante este hallazgo ya conocido ES el resultado esperado) y no se intentó ninguna variación adicional de productos, por instrucción explícita del usuario. Retomar este gap cuando se confirme que el ambiente fue corregido (repetir primero la verificación mínima de solo lectura de esta sección con un producto simple conocido).
...
## 25. Crear Cliente desde el POS (CP-193, CP-195, CP-197–CP-199, 2026-07-25/26)

Explorado en vivo el modal completo de creación de clientes del POS, hasta ahora sin cobertura dedicada (los CPs previos que asocian cliente al carrito usan el atajo "Nombre rápido" — `editQuickCustomerName()`/CP-034/CP-182 — que es un mecanismo DISTINTO y mucho más limitado).

### Cómo se abre
Junto al buscador `#search_pos_customer` del carrito hay un botón **"Agregar"** (`btn btn-success dropdown-toggle`) que despliega un menú de 2 opciones: **"Nombre del cliente"** (`id=""`, `onclick="editQuickCustomerName()"` — el atajo ya documentado) y **"Nuevo Cliente"** (`id="add_quick_customer"`, sin `onclick` inline) — esta segunda abre el modal completo `#dialog_add_customer` ("Agregar Cliente").

### Estructura del modal — 3 tabs (`li_form_customer_step1/2/3`)
- **PRINCIPAL**: `c_identifier` (Identificación) + botón lupa (búsqueda, no explorado a fondo) · `c_identification_type` (select, "Seleccione.../Cédula Física/Cédula Jurídica/DIMEX/NITE/Extranjero No Domiciliado") · `c_name` (Nombre, **único campo requerido del modal completo**) · `c_email` · `c_principal_economic_activity` (select) · botón **"+ Actividad"** (`a.btn_activity`, texto exacto "Actividad" — las filas ya agregadas tienen un botón hermano `.btn_activity` de texto vacío para eliminar, `onclick="remove_customer_modal_secundary_activity(N)"`) que inserta un `<select id="secundary_activity_sector_N">` nuevo por cada clic (N=1,2,3...), permitiendo agregar TANTAS actividades secundarias como se quiera · `c_code` (Código del cliente) · `c_batch` (Batch) · `c_province_pos_modal` (Provincia) · `c_address` (textarea Dirección) · `c_whatsapp` / `c_telefono_1` (ambos con prefijo fijo "+506") · switch **`checkbox_more_information_car_add_customer`** ("Agregar o ver información del vehículo") que revela una sub-sección "Información de vehículo" con 6 campos: `c_plate_number` (Número de Placa), `c_unit_number` (Número de caso), `vehicle_brand` (Marca, select), `vehicle_model` (Modelo, select **encadenado a Marca vía AJAX** — ver hallazgo abajo), `vehicle_year` (Año, select) y `c_vehicle_chassis` (Número de chasis) + botón **"Agregar"** (scoped al modal — hay otro botón "Agregar" global en el header del POS, no confundir) que agrega el vehículo a una tabla propia (Placa/Número de caso/Marca/Modelo/Año/Número de chasis + botón eliminar) y limpia los 6 campos para poder agregar otro vehículo.
- **OPCIONES AVANZADAS**: switch `ck_is_exempt` ("Exento de pago impuestos de venta en un 100%") · `c_agent` (Vendedor Asignado) · `c_zone` (Zona del cliente) · `c_route` (Ruta del cliente, **multi-select**) · `c_default_document_type` (No aplica/Factura Electrónica/Tiquete Electrónico/Factura Interna) · `c_paydate` / `c_trammitdate` (Días de pago / Días de trámite, ambos **multi-select** de días de la semana) · `c_recurrence` (Semanal/Quincenal/Mensual) · `c_limit` (Límite de crédito, number).
- **UBICACIÓN**: mini-formulario "Agregar Nueva Dirección" — `c_address_name` (Lugar) · `c_written_address` (Dirección escrita) · switch `c_default_address` ("Usar por defecto") · `c_address_url` (opcional, Waze/Maps) · botón **"Agregar dirección"** que agrega la fila a una tabla "Direcciones Guardadas" (Lugar/Latitud/Longitud/Dirección/Por Defecto) y muestra un toast **"Dirección agregada. Se guardará al crear el cliente."** — mismo patrón "staged, se persiste solo al Guardar y Salir" que el vehículo.

Navegación entre tabs: botones **"Siguiente"**/**"Anterior"** (`next_form_customer_step()`) o clic directo en las pestañas — **ambos caminos son bloqueados por validación si "Nombre" está vacío** (el clic en la pestaña destino simplemente no hace nada y muestra el tooltip rojo "Campo requerido" bajo Nombre, sin lanzar ningún error de JS) — llenar Nombre SIEMPRE antes de intentar cambiar de tab.

### Guardado y verificación
Botón **"Guardar y Salir"** (`save_customer()`) — al confirmar: (1) aparece un toast **"Cliente guardado correctamente!"**, (2) el modal se cierra, y (3) el cliente recién creado se **asocia automáticamente a la venta actual del POS**, reemplazando el panel `#search_pos_customer` por un bloque `Buscar Cliente | Código: ... | Cliente: ... | Crédito Disponible: ... | Correo: ... | Dirección: ... | Placa: [Seleccionar Placa ▾] [+]` — este bloque es la forma más confiable de verificar el guardado (más rápido y menos frágil que reabrir el cliente). Si el cliente tiene vehículo(s) asociado(s), su placa queda disponible de inmediato en ese selector "Placa" del panel de venta (confirmado en CP-197). Junto al "Código:" hay un ícono de edición real, clase **`.i_edit_customer`** (`<i class="fa fa-pencil i_edit_customer">`, sin id, ubicado ~20px a la derecha de "Código:") que reabre `#dialog_add_customer` PRE-POBLADO con todos los datos del cliente — la forma más robusta de verificar que los 3 tabs (no solo lo visible en el panel de venta) persistieron realmente en el backend.

### Hallazgos técnicos de automatización
- **Todos los `<select>` de "Opciones avanzadas" (`c_agent`/`c_zone`/`c_route`/`c_default_document_type`/`c_paydate`/`c_trammitdate`/`c_recurrence`) son en realidad widgets Chosen** (confirmado en vivo inspeccionando el hermano DOM de cada uno: `chosen-container-single` para los de selección única, `chosen-container-multi` para `c_route`/`c_paydate`/`c_trammitdate`) — la clase CSS `selectpicker` que llevan es vestigial/engañosa, NO son bootstrap-select. El multi-select de Chosen se abre con `.chosen-choices` (no `.chosen-single`, que es solo para single-select) — un selector genérico debe probar ambos disparadores.
- **El `<select>` de Modelo depende de Marca vía una llamada AJAX real, no instantánea** — un `waitForTimeout` fijo corto (600ms) no siempre alcanza; hay que esperar activamente (`waitForFunction` a que `options.length > 1`) antes de leer/usar las opciones. Además, **no todas las marcas tienen catálogo de modelos poblado en este ambiente QA** — "Toyota" quedó con 0 modelos en las pruebas (confirmado, `<select>` de Modelo permanece vacío tras elegirla), mientras que **"AUDI" sí tiene un catálogo completo (32 modelos: A1, A3, A4...)** y es la marca recomendada para usar en CPs nuevos que necesiten un vehículo real.
- **Verificar por `document.body.innerText` en vez de filtrar elementos por `getBoundingClientRect()` uno por uno** — al confirmar el panel de cliente tras guardar, un filtro `Array.from(document.querySelectorAll('*')).filter(isVis)` dio falsos negativos repetidos (el dato SÍ estaba visible en pantalla, confirmado por screenshot, pero no se encontraba vía ese filtro) por timing/layout de contenedores flex; `document.body.innerText.includes(texto)` fue consistentemente confiable y más simple.
- **El teléfono/Whatsapp vuelven con máscara al reabrir** ("8888 1111" en vez de "88881111" tal como se escribió) y el **Límite de crédito vuelve como decimal** ("50000.000000") — normalizar (quitar espacios, `parseFloat`) al comparar contra el valor original en vez de comparar el string tal cual.
- El ícono de editar (`.i_edit_customer`) NO tiene ningún atributo `onclick` ni clase con "edit"/"pencil" en el nombre reconocible por un selector genérico basado en texto/clase típica (`fa-edit`, `edit` como palabra) — solo se encontró inspeccionando visualmente la región del panel y volcando todos los `<i>` con sus coordenadas.
...
## 27. Módulo "Ventas" — explorado, hallazgo crítico escalado a registros PERSISTIDOS, cubierto con CPs de solo lectura (CP-208–CP-213, 2026-08-01/02)
...
### Decisión inicial (2026-08-01) — bloqueado, sin CPs de cobertura "feliz"
Por instrucción explícita del usuario: al confirmarse que el módulo está afectado por el hallazgo crítico, en un primer momento **no se escribió ningún CP** para "Ventas" — se documentó el hallazgo (esta sección) y se preguntó al usuario cómo proceder, mismo criterio que con el bloque de combos (`CP-200`, sección 22) y el bloque de "retomar proforma" (pausado desde el 19/07/2026).

### Decisión final (2026-08-02) — Opción 2: CPs de "solo lectura", sin validar ningún monto
El usuario eligió cubrir el módulo con CPs de **solo lectura**: navegación, filtros y búsqueda de las 6 sub-pantallas, sin leer ni comparar ningún monto/cifra en ningún momento. Carpeta nueva `tests-playwright/08-ventas/01-navegacion-filtros/` (el top-level `07-citas/` ya estaba ocupado por otra sesión, ver sección 28).

**⚠️ Segunda colisión de numeración en el mismo bloque**: los primeros 3 CPs se numeraron inicialmente CP-205/206/207, pero para cuando se fue a confirmar el commit, otra sesión concurrente ya había usado esos mismos 3 números para el módulo "Citas" (`cp205-ver-detalle-cita.js`, `cp206-editar-cita.js`, `cp207-cancelar-cita.js`, sección 28). Se detectó al revisar `git status`/el árbol de `tests-playwright/` justo antes de documentar, y se renumeraron en caliente a **CP-211/CP-212/CP-213** (re-ejecutados y reconfirmados 5/5, 5/5 y 3/3+hallazgo tras el renombre) — CP-208/209/210 no colisionaron y se dejaron con su numeración original. Resultado: la numeración final de este bloque queda **CP-208, CP-209, CP-210, CP-211, CP-212, CP-213** (no consecutiva desde 205 — anotado aquí explícitamente para que la próxima sesión no asuma un rango limpio).

### Ayudante de navegación común a los 6 CPs
`irASubitemVentas(page, textoSubitem)`: hace clic en el acordeón lateral **"Ventas"** (`href="javascript:void(0);"`, distinguir del link de WhatsApp homónimo) y luego en el sub-ítem por texto exacto. **Trampa descubierta**: si el acordeón ya está abierto (ej. al re-navegar dentro del mismo CP para resetear filtros contaminados), volver a clickear "Ventas" lo **cierra** en vez de abrirlo — el helper primero comprueba si el sub-ítem ya es visible y solo clickea "Ventas" si hace falta.

### Selectores y hallazgos por sub-pantalla
- **Histórico de Ventas** (CP-211, `/receip/printPosReceip`): filas `.receip_item`. Tabs de estado por texto exacto (Todas/Contado/Crédito/Pendientes/Anuladas) — **"Anuladas" NO pertenece al mismo grupo mutuamente excluyente que Todas/Contado/Crédito/Pendientes** (confirmado en vivo: tras clickear Anuladas, "Todas" no vuelve a su conteo original) — no comparar su conteo contra "Todas". `#start_date`/`#end_date` (date nativos) + `#receip_search`, que **requiere `Enter`** tras `fill()` (un `fill()` + esperar no dispara la búsqueda por sí solo).
- **Abono Cuentas por Cobrar** (CP-212, `/credit_sale/clientCreditSales`): botones `#state_pending`/`#state_paid`. El conteo de filas (proxy: botones `[id^="btn_pay_customer_invoice_"]`) **no cambia entre ambos estados** en los datos de este ambiente (dio el mismo número en Pendientes y Canceladas) — se valida con `page.waitForResponse` (consulta real al servidor) en vez de con el conteo. `#search` + botón `#btn_search` (sí necesita el botón, `Enter` solo no alcanza). **No se hace clic en "Abonar" (`btn_pay_customer_invoice_N`) ni en el detalle (`btn_get_customer_detail_N`)** — abren un flujo de pago real, fuera de alcance de "solo lectura".
- **Lista de Cobros** (CP-213, `/receip/receivableList`): filas `.brand-card` (distinta de `.receip_item`). Botones de día `show_list_1..7` (Lunes..Domingo) — **no existe un botón "todos los días"** para deshacer el filtro; quedarse en un día sin datos (ej. Viernes, 0 filas) contamina cualquier paso posterior — hay que re-navegar (recargar el sub-ítem) para resetear, no basta con re-clickear otro día. Botones de estado `show_list_status_0/1/2` (Todos/Pendientes/Abonados). Dropdown "Moneda: Todas" (`button.dropdown-toggle`) con opciones reales (CRC/USD/EUR/DOP). **⚠️ Hallazgo confirmado**: `#receivable_search` no tiene `oninput`/`onkeyup`/`onchange` ni botón de búsqueda asociado (el ícono de lupa contiguo no tiene `onclick`) — escribir en él y presionar Enter NO filtra el listado; documentado como hallazgo (⚠️), no como fallo del CP.
- **Historial Mov. de Caja** (CP-208, `/cash_movement/movements`): **0 movimientos reales en este ambiente QA** (confirmado, área principal vacía) — los filtros (`#start_date`/`#end_date`/`#receip_search`) se validan por `page.waitForResponse` (consulta real al servidor tipo POST), no por cambio de conteo de filas (imposible de observar con 0 datos). El filtro de fecha necesitó reforzar con `dispatchEvent('change')` + `blur()` explícitos tras el `fill()` del segundo campo para disparar la consulta de forma confiable.
- **Devoluciones** (CP-209, `/refund/refund`): `#refund_invoice_search`, `#refund_start_date`/`#refund_end_date`. Mismo patrón de `dispatchEvent('change')`+`blur()` para el filtro de fecha. **No se hace clic en "Agregar" (`#btn_add_refund`)** — crea una devolución nueva, fuera de alcance.
- **Nota de crédito** (CP-210, `/creditNote/creditNote`): filas = botones `.nc-dot-btn` (uno por nota listada). `#nc_compensation_filter` (select nativo: Todas/Reembolso dinero/Saldo a favor/Contra factura/Anulación) sí acota el listado de forma observable (24 → 7 filas al elegir "Reembolso dinero" en las pruebas). `#receip_search` (placeholder "NC, cliente, factura...") sí funciona correctamente en esta pantalla (a diferencia de Lista de Cobros). **No se interactúa con el dropdown "Acciones" (`#btn_credit_note_actions`) ni los menús "..." por fila (`.nc-dot-btn`, reutilizados aquí solo como conteo, nunca clickeados)** — disparan flujos reales de anulación/devolución.

### Estado de implementación — ✅ completo dentro del alcance de "solo lectura" (2026-08-02)
6 CPs (CP-208 a CP-210, CP-211 a CP-213), todos pasando (5/5, 5/5, 4/4, 4/4, 5/5, y 3/3+1 hallazgo documentado respectivamente) — navegación real por menú, filtros y búsqueda validados a nivel de interfaz en las 6 sub-pantallas, **sin leer ni comparar ningún monto en ningún momento**, tal como exigía el hallazgo crítico activo. Deliberadamente NO cubierto (fuera del alcance de "solo lectura" autorizado): cualquier acción que modifique datos (Abonar, Agregar devolución, Acciones/anulación de nota de crédito) y, por supuesto, cualquier validación de montos/totales — eso sigue bloqueado hasta que se resuelva el hallazgo de la sección 22. Ver `PLAN-PROYECTO-FINAL.md` punto 12.
...
### 🔴 PAUSADO — Servicios y Productos / Convertir a orden (mismo motor de precios corrupto, sección 22)
La sección "Servicios y Productos" del formulario tiene buscadores reales (`#search_services_reservation`, `#search_product_reservation`) y muestra **"Total Servicios:", "Total Productos:" y "Total General:"** — es decir, agregar un producto/servicio a una cita SÍ dispara el mismo cálculo de montos que el carrito del POS, potencialmente expuesto al mismo bug. Además, el modal de detalle de cada cita tiene un botón **"Convertir a orden"** que crea una orden de taller real a partir de la cita (ambos dependen del mismo motor de precios que ya está confirmado corrupto tanto en el carrito en vivo — sección 22 — como en registros persistidos — sección 27). **Decisión del usuario (2026-08-02): pausado, mismo criterio que "retomar proforma" (sección 22) y el módulo "Ventas" (sección 27) — no se investiga más por ahora, ningún CP de cobertura para estas 2 acciones.** El resto del ciclo de una cita (crear/ver/editar/cancelar) es genuinamente independiente del cálculo de montos — confirmado en CP-205, que verifica que "Total General" se mantiene en ₡0.00 mientras no se toque esa sección.
...
### Asignar a (mecánico), Recurrencia y Estado — SÍ son `<select>` reales, ocultos tras un widget custom
A diferencia de lo que parecía en la primera exploración, los 3 campos del acordeón "Recurrencia" (`.ap-company-dropdown` es solo el contenedor visual) tienen un `<select>` nativo oculto (`style="display:none"`) detrás, con options reales — se pueden setear igual que cualquier select del proyecto (`.value = X` + `dispatchEvent('change')`), sin tocar el widget visual en absoluto: `#assigned_to_reservation` (17 opciones, empleados/mecánicos reales — ej. value="321" text="valentina mecanico prueba", el mismo mecánico de prueba usado en otras partes del proyecto), `#recurrence_reservation` (0=Ninguna, 1=Semanal, 2=Quincenal, 3=Mensual), `#status_reservation` (1=Activo, 2=Suspensión). Todo confirmado persistido correctamente vía la respuesta de `view_reservation` (campos `assigned_to`, `assigned_name`, `recurrence_reservation`, `status_reservation`).

### Hallazgo — vincular un vehículo NUEVO a una cita requiere su propio widget, no un `<select>` simple
`#vehicle_reservation_brand` (Marca) SÍ es un `<select>` real seteable igual que los anteriores, pero **el sistema exige que si se llena la Placa, también se seleccione Marca Y Modelo juntos** — validación confirmada en vivo con el toast bloqueante *"¡Si ingresas la placa del vehículo, también debes seleccionar la marca y el modelo!"* que impide el guardado (`save_reservation` ni siquiera se dispara). El campo Modelo (`#vehicle_reservation_model`) depende de una librería propia (`vehicle_brand_model.core.js`/`.data.js`, assets estáticos, sin llamada AJAX por marca) que no responde a un simple `.value` + `dispatchEvent('change')` genérico — requiere interactuar con su propio widget visual. La app ofrece un atajo real para este caso: al intentar guardar con una placa sin marca/modelo completos para un cliente que ya tiene vehículos registrados, se abre un modal **"Seleccionar Vehículo"** con tarjetas de los vehículos existentes del cliente (Placa/Modelo/Estilo/Unidad) — clickear una tarjeta completa la selección de una sola vez. **No se automatizó** (ni la vinculación de vehículo nuevo vía el widget propio, ni el atajo de "Seleccionar Vehículo") — se documenta como hallazgo/límite conocido, dado que crear una cita completa SIN vehículo (CP-204/205/206/207) o solo con Placa/Año como texto simple ya cubre el ciclo real de negocio; vincular marca/modelo exactos no es parte del ciclo mínimo de una cita.

### Límite conocido — WhatsApp / Email en el detalle de la cita, NO se ejercen (decisión del usuario)
Los botones "WhatsApp" y "Email" del modal de detalle (`confirm_send_whatsapp(this)` / `confirm_send_email('<id>','<email>')`) no son enlaces informativos — están cableados para confirmar y disparar un envío real de notificación al cliente (WhatsApp/correo). A diferencia de crear/ver/editar/cancelar, esta es una acción con efecto real hacia un canal externo (aunque sea la bandeja de prueba del propio proyecto), fuera del patrón estándar de la suite — **decisión del usuario (2026-08-02): no se ejecuta**, mismo criterio que la vinculación de vehículo nuevo (abajo). Documentado como límite conocido, no como pendiente.

### Estado de implementación — ✅ completo dentro del alcance autorizado (2026-08-02)
5 CPs (`tests-playwright/07-citas/01-crear-editar-citas/`), todos con sus validaciones en verde: **CP-204** crea una cita básica y confirma por red; **CP-205** ve el detalle de una cita vía Agenda y confirma acciones reales + Total General ₡0.00; **CP-206** edita una cita existente confirmando que persiste sobre el mismo id; **CP-207** cancela una cita confirmando el soft-delete (`is_active=0`); **CP-214** asigna mecánico + recurrencia + estado y confirma persistencia por red. **Pausado por decisión del usuario** (mismo criterio que secciones 22/27): agregar servicios/productos a una cita y "Convertir a orden". **Sin automatizar, documentado como límite conocido** (no como pendiente, ambos por decisión del usuario): vincular un vehículo nuevo a una cita (requiere el widget propio de marca/modelo o el atajo "Seleccionar Vehículo") y los botones WhatsApp/Email del detalle (acción con efecto real hacia un canal externo, fuera del patrón estándar de crear/ver/editar/cancelar).
...
## 31. Recepción de Vehículo — Bloque "Creación de Recepción" (CP-256–CP-261, 2026-08-17/19)

Wizard completo de "Nueva orden de reparación" (`/vehicularReception/vehicularQuickReception`, botón `button.add-reception-btn`), no cubierto por CPs hasta ahora salvo el acceso básico (CP-006). Se cubrieron las 3 variantes de creación de vehículo y 3 recorridos end-to-end del wizard completo con profundidad creciente (sencilla/completa/avanzada). **Nota**: distinto del wizard "End. Pintura" dentro del **POS** (sección 20) — aunque ambos comparten el mismo patrón visual Tipo de vehículo→Parte→Pieza→Servicio, son implementaciones separadas con selectores propios (`#type_car_select` acá vs. `#select_type_vehicle` en el POS).

### Modal de ingreso de vehículo (`#dialog_search_vehicle_by_plaque`)
- Input placa: `#vehicle_plaque`. Botón "Agregar vehículo": `#vr_add_vehicle_btn`. Botón "Buscar" (por placa existente): `#btn_modal_search_vehicle`. También existe una opción "Buscar por VIN" (`#vehicle_vin_search`, oculta por defecto tras `#btn_search_by_vin`).
- Toggle "No tiene placa / matrícula": el checkbox real es `#vehicle_has_plaque_check` (envuelto en un `<label>`) — clickear su texto (`#vr_no_plaque_label`) o el propio checkbox, **no** un contenedor genérico por texto (ambigüedad de varios nodos coincidentes).
- Reutilizar una placa ya registrada en una **nueva** recepción no muestra ningún diálogo de conflicto — el sistema simplemente crea una segunda orden con la misma placa y avanza normalmente a "Seleccionar Cliente".

### Estructura confirmada del wizard (barra lateral "Pasos de Recepción")
Seleccionar Vehículo → Seleccionar Cliente → Seleccionar estilo → Detalles del vehículo → Seleccionar servicios → Inspección → Enderezado y Pintura → Abonos → Partes del vehículo → Seleccionar fotos → Marcación de daños → Observaciones generales → Firma del cliente → (Generar). Navegación: `#btn_next_step_reception` ("Siguiente"), `#btn_back_step_reception` ("Anterior"), `.btn-return.btn-modern` ("Regresar a órdenes"). **Cada `.card-step` de la barra lateral está duplicado en el DOM** (una versión solo-ícono sin texto + una con texto) — al buscar "el paso activo" por clase `active`, filtrar por `textContent` no vacío o se obtiene el duplicado vacío.

### Seleccionar Cliente
Buscador `#vehicular_reception_customer_search` + botón `.toolbar-btn.search-btn`; "+ Agregar Cliente" (formulario completo) es `.toolbar-btn.add-btn` → modal con `cf_identifier`, `cf_name`, `cf_email`, `cf_code`, `cf_batch`, `cf_address`, `cf_whatsapp`, `cf_phone_1`, `cf_vehicle_toggle`. Cada tarjeta de cliente tiene un botón `.customer-options-btn` que revela "Editar cliente" y "Seleccione el cliente" — este último **avanza el wizard automáticamente** a "Seleccionar estilo" sin necesidad de clickear "Siguiente" aparte.

### Seleccionar estilo
Buscador con placeholder `"Buscar..."` + botón "Buscar" (sin id fijo, ubicar por proximidad al input) + `"+ Agregar Estilo"`. Cada tarjeta de estilo real es `.style-vehicle` (`onclick="setVehicleStyle(id)"`) con dos botones superpuestos: `.edit-btn` (editar, ícono `fa-pencil`) y `.select-btn` (seleccionar, ícono `fa-hand-pointer-o`) — clickear `.select-btn` avanza automáticamente a "Detalles del vehículo". **Cuidado**: búsquedas genéricas por `img + [onclick]` en toda la página capturan widgets flotantes de soporte ajenos (`onclick="openMessage(...)"`, "Enviar una idea" / "Comunícate con el CEO") que también tienen imagen y onclick — filtrar específicamente por la presencia de `.edit-btn`+`.select-btn` o escopar al panel de resultados.
- Modal "Agregar Estilo" = "Administrador de estilos": `#vehicle_admin_style_name` (Nombre), `#vehicle_admin_style_type` (Tipo: Hatchback/Crossover/Minivan/SUV/Automóvil/Pick-up/CAMIONETA — requerido, sin este campo el guardado no persiste), imagen opcional (`#vehicle_style_photo`, con placeholder por defecto si no se sube nada).
- **Nota de timing**: un estilo recién creado (endpoint `addVehicleStyleReception`, responde `200` + ID numérico) tarda varios segundos en volverse buscable en el catálogo — no es un hallazgo de bug, es indexación asíncrona. Buscarlo inmediatamente tras crearlo da falso negativo; hace falta reintentar la búsqueda con espera (confirmado: el mismo estilo SÍ aparece unos segundos después).

### Detalles del vehículo
Campos: Placa (prellenada), **Marca** (`#vehicle_brand`, Chosen, único campo con gate de validación duro: toast "Debe seleccionar la marca del vehículo"), **Modelo** (`#vehicle_model`, Chosen, dependiente de Marca vía AJAX — algunas marcas no tienen modelos cargados), Número de caso, Año (con año actual por defecto), Tipo de vehículo (`#reception_vehicle_type`: combustión/híbrido/eléctrico), Sucursal, Tipo de combustible, Transmisión — los últimos 5 con botón "+" al lado para agregar una opción nueva sin salir del formulario. Color primario/secundario (color pickers), `% de la batería` (relevante solo si Tipo de vehículo = eléctrico/híbrido), medidores decorativos de combustible/temperatura. Dos acordeones colapsables: "Carrocería" y "Aseguradora" (no se llegó a completar su contenido en detalle en este bloque — pendiente si se retoma este flujo).

### Seleccionar servicios (reutiliza el wizard interno ya documentado en otros módulos de este proyecto)
- "Agregar producto" (`p:visible` con texto "Agregar producto" — **duplicado en el DOM**, usar `.first()`) abre un SweetAlert2 con 2 opciones de **id fijo**: `#quick-service` (Producto Rápido) y `#normal-service` (Producto, todas las opciones) — **no** filtrar por texto "Producto"/"Rápido" (ambiguo, "Producto Rápido" contiene la palabra "Producto").
- Modal "Agregar Producto" (`#dialog_add_quick_product`): wizard interno **jQuery Steps** de 2 tabs (Inf. General/Costos). Sus botones de navegación **no son `<button>`**, son `<a href="#next">`/`<a href="#save">`/`<a href="#finish">` dentro de `.actions.clearfix` — en el último paso, `#save` queda oculto (`display:none`) y el enlace real para terminar es `#finish` ("Finalizar"). Campo Nombre: `#product_name_app`. Botón CABYS (gate obligatorio) abre `#cabys_code_search` → fila con botón `.btn-success` "APLICAR". **Categoría y Proveedor son selects requeridos** (`category_select_spk_app`/`provider_select_spk_app` en esta instancia — pueden variar el sufijo numérico, ubicar por fragmento de id `categor`/`provider`) sin los cuales el wizard no avanza de tab. **La cantidad debe ser mayor a 0** para que el producto aparezca luego en el buscador del catálogo de este paso (bodega-filtrado) — con cantidad 0 el producto se crea pero es invisible en la búsqueda.
- Para agregar el producto creado al carrito: buscarlo por nombre en `input[placeholder="Buscar productos"]`, clickear su tarjeta real (`.product_box.card-product`, `onclick="add_item_to_repair_order(...)"`) — clickear la tarjeta dispara automáticamente un modal "Asignar mecánico" que debe cerrarse (o usarse para asignar, ver CP-261) antes de continuar.
- Carrito real: `.service-list-container .item-card` (contar con `document.querySelectorAll(...).length`) — **no** es una `<table>` a pesar de que la cabecera muestra columnas "CANTIDAD/PRECIO UNIDAD/TOTAL"; buscar una `<table>` con ese texto siempre devuelve 0 (confirmado, hallazgo de sesiones previas ya corregido en estos CPs).
- Cada ítem del carrito tiene un botón `G` (garantía) visible junto al precio — clickearlo no dejó el precio del ítem en 0 en las pruebas realizadas (ver hallazgo CP-261 abajo).
- Crear un **servicio** nuevo: tab "Servicios" del catálogo (junto a "Productos") → "Agregar servicio" → mismo patrón de opción rápida/normal → modal con input `#dialog_service_name`. El servicio creado, igual que el estilo, no siempre queda inmediatamente agregable/visible tras crearlo (mismo patrón de indexación asíncrona).

### Enderezado y Pintura
Select real "Tipo de vehículo": **`#type_car_select`** (Chosen, mismo catálogo de opciones que Estilo: Hatchback/Crossover/Minivan/SUV/Automóvil/Pick-up/CAMIONETA×2). Al elegir un tipo aparecen tarjetas de "Parte" (ej. "Parte frontal"/"Parte trasera", con imagen — click en la tarjeta, no en el ícono de búsqueda del encabezado "Piezas" 🔍 que por error de selector genérico se puede confundir con una opción real ya que su texto interno es literalmente la palabra "search", propio de fuentes de íconos). Tras elegir una Parte se listan Piezas, y tras elegir una Pieza se listan Servicios — ambos paneles muestran una única opción "Agregar pieza"/"Agregar servicio" cuando el catálogo está vacío para esa combinación específica.
- **⚠️ HALLAZGO (no bloqueante, CP-260)**: cuando el catálogo de piezas/servicios está vacío para la combinación Tipo de vehículo + Parte probada (confirmado con SUV + Parte frontal), crear una pieza/servicio nuevo vía sus modales respectivos ("Agregar pieza"/"Agregar servicio", solo campo Nombre) no deja la pieza/servicio recién creado seleccionable en el panel — sigue mostrando únicamente la opción "Agregar...". El conteo del carrito (`.service-list-container .item-card`) no aumenta. No se identificó si falta algún campo adicional requerido en esos modales (a diferencia de Estilo, que sí requiere explícitamente un "Tipo"). Pendiente de investigación futura si se retoma este flujo con más profundidad.

### Abonos
Monto: `#initial-payment-repair-order` (number). Forma de pago: `#select_payed_with_ro`. Caja: `#apply_to_cash_id`. Botón "Guardar". **Cuidado al elegir la primera opción real de estos selects**: su opción placeholder (índice 0) a veces repite el texto de la propia etiqueta del campo ("Forma de pago", "Caja") en vez de decir "Seleccione..." — filtrar por índice (`options[0]` siempre es el placeholder, sin importar su texto), no por regex de texto tipo `/seleccion/i`.

### Partes del vehículo
Grid de tarjetas, cada una con 4 íconos de estado (buena/regular/mala/no tiene, verde/naranja/rojo/triángulo de advertencia) y un contador `- Cantidad +`. Buscador `#asset_search_input`.

### Seleccionar fotos / Marcación de daños / Observaciones / Firma
- Fotos: zona de arrastrar-y-soltar + `input[type="file"]`, botón "Eliminar todas". Si hay al menos un servicio en el carrito, aparece una sección "Servicios" propia debajo para fotos antes/después por servicio.
- Marcación de daños: canvas con diagrama de vehículo (6 vistas), selector de `COLOR`, `TAMAÑO PUNTERO` (5 tamaños), "AGREGAR TEXTO" (`Colocar texto` + tamaño de fuente +/-), botones "Importar imagen"/"+ Nueva"/"Guardar nueva"/"Actualizar"/"Eliminar"/"Limpiar", toggle "Mostrar en el PDF e impresiones".
- Observaciones generales: dos textareas, `#damage_repair` (observaciones extensas) y `#damage_repair_message` (notas del cliente, cortas).
- Firma del cliente: `<canvas>` de firma, paleta de 7 colores + borrador, botón "Limpiar firma", indicador "Listo para firmar". Es el **último paso** — el botón "Siguiente" se reemplaza por uno verde "Generar".

### Generar orden
Clic en "Generar" abre un diálogo de confirmación ("¿Está seguro de generar la orden?", botones "Cancelar"/"Generar orden" — **no** es el mismo click, hace falta confirmar dos veces). Tras confirmar, aparece automáticamente el modal "¡Enviar WhatsApp!" con el número de orden real generado (ej. "Orden #949") — esa aparición es la señal de éxito más confiable (mejor que revisar la URL, que no cambia de ruta base).

### 🔴 HALLAZGO CRÍTICO CONFIRMADO — "+ Crea Nuevo Paquete" (paso Inspección) no funciona (CP-261)
En el paso "Inspección" (`Lista de inspección de la orden`), cuando no hay paquetes agregados se muestra el botón **"+ Crea Nuevo Paquete"**. Al hacer clic (confirmado con un clic **nativo de Playwright**, no vía `evaluate`, descartando un problema de simulación de eventos) **no ocurre absolutamente nada observable**: sin modal, sin toast, sin error de consola ni de página (`page.on('console')`/`page.on('pageerror')` verificados vacíos), y un screenshot antes/después es pixel-idéntico. El botón "+ Crear Nuevo Componente" (columna derecha) depende de tener un paquete creado primero, por lo que tampoco se pudo ejercer.

**Pasos para reproducir**: 1) Nueva orden de reparación → vehículo/cliente/estilo/detalles mínimos. 2) Avanzar hasta el paso "Inspección" (con o sin ítems en el carrito de Servicios). 3) Clic en "+ Crea Nuevo Paquete". **Esperado**: modal para nombrar el paquete nuevo. **Real**: sin cambio de estado observable.

No se pudo cubrir ningún flujo de "crear paquete de inspección y componente" en este bloque como resultado directo de este hallazgo — documentado, no forzado.

### Estado de implementación — ✅ cubierto dentro del alcance de Bloque 1 "Creación de Recepción" (2026-08-19)
6 CPs en `tests-playwright/02-gestion-taller/01-recepcion-vehiculo/` (carpeta ya existente junto a CP-006–CP-016): **CP-256** (placa nueva), **CP-257** (sin placa), **CP-258** (placa existente), **CP-259** (orden sencilla completa), **CP-260** (orden completa, + Enderezado y Pintura + Abonos) y **CP-261** (orden avanzada, + servicio nuevo/mecánico/garantía/eliminar + hallazgo de paquetes de inspección — ver arriba).

⚠️ **Nota de concurrencia**: durante esta sesión se detectó que otra sesión trabajó en paralelo el mismo encargo, compartiendo el mismo directorio de trabajo físico (no worktrees aislados), y ya había commiteado su propia versión de CP-256–CP-260 (commit `ef395fd`) junto con CP-291–CP-293 (Cliente/Estilo/Detalles del vehículo, con enfoque granular por paso) y CP-300. Para no descartar trabajo ya entregado por esa sesión, se restauró su versión commiteada de `cp260-orden-completa.js` (que había quedado sobrescrita localmente sin commitear) y se dejaron sus CP-256–CP-260 tal cual estaban. **CP-261 es la única pieza exclusiva de esta sesión** (no existe en el commit paralelo) y se agrega sobre lo ya existente. Los hallazgos técnicos documentados arriba (selectores del wizard, hallazgo de Enderezado y Pintura, hallazgo crítico de "Crea Nuevo Paquete") fueron confirmados mediante interacción real en vivo contra la aplicación y son válidos independientemente de qué archivo de prueba específico los ejerce.

Parte B del encargo original (flujo "Ver Orden": facturar/regresar/editar cliente desde la orden/histórico de vehículo) quedó **fuera de alcance de esta sesión** por ser explícitamente condicional ("si sobra tiempo") frente al volumen ya cubierto en la Parte A.

---

## 32. Recepción de Vehículo — Bloque "Flujos Básicos" (CP-300, CP-301, CP-310–CP-313, 2026-08-19)

Encargo separado del bloque 31 (que cubre el *wizard* de creación paso a paso): este bloque cubre los flujos **generales del Panel de Recepción** — el nivel "Torre de Control" (`/vehicularReception/vehicularQuickReception`) con sus tabs, configuración y acciones sobre órdenes ya existentes, excluyendo explícitamente lo ya cubierto en la sección 30 (compartir WhatsApp/correo, QR, ver orden online, documentos PDF, abonos, selección múltiple).

### Tabs reales del panel (confirmado en vivo, versión 9.6.72)
`Tablero`, `Órdenes`, `Repuestos`, `Cotizaciones`, `Gráficos`, `Tabla informativa` — **"Dashboard" y "Citas" NO existen como tabs de este panel** en esta versión (buscados explícitamente, confirmado con query vacío). Cada botón de tab tiene texto prefijado por un ícono, lo que rompe cualquier match exacto de texto — usar `.includes(t) && length < t.length+30`, ordenando por el más corto, no `===`.

### CP-300 — Reporte de órdenes + Admin. WhatsApp
Kebab del módulo (`#dLabel1299`) → link `/reporte de órdenes/i` navega a `/reports/order_report`. "Admin. WhatsApp" (`show_dialog_whatsapp_manager(0)` → modal `#dialog_whatsapp_manager`): Agregar vía `#txt_shortcut`/`#txt_message` + `update_whatsapp_message()`; Buscar vía `#input_dialog_whatsapp_manager` + `get_whatsapp_message()`; Editar vía botón `onclick*="show_hide(0,<id>"`; Eliminar vía `confirm_delete(id, 0)` + confirmación SweetAlert "Eliminar". CRUD completo validado, autolimpia su propio mensaje de prueba.

### CP-301 — Carga de tabs + buscador
Los 5 tabs reales cargan datos verificables (longitud de contenido + ausencia de texto de error 500). El buscador `#repair_order_search` filtra resultados tanto en tab "Tablero" como en "Órdenes".

### CP-310 — Modo lista / cuadrícula
Toggle entre vista de lista y cuadrícula vía íconos `.fa.fa-th-large`/`.fa.fa-list`; el cambio de vista se refleja en la clase del contenedor (`view-repair-order-list` ⇄ estado `btn-success-innactive`). Ejercido en ambas direcciones.

### CP-311 — "Modo oscuro" (ausente) + Configurar tablero
**No existe ningún control de "modo oscuro"** en este ambiente — revisado el menú de cuenta ("Design Soft" → Mi perfil/Cerrar sesión) y el panel global `#header_control_panel_open` (160 switches, 11 categorías, incluyendo "Empresa y cuenta" específicamente). El modo claro es, por ausencia de alternativa, el único/default de forma trivial. "Configurar tablero" (kebab `#dLabel1299` → link `/Configurar tablero/i` → modal con `.ervk-card-mode-option` "Compacto"/"Detallado" + botón `#ervkSaveSettings`) sí funciona: el cambio persiste tras recargar la página. **Importante**: el CP restaura el modo original ("Compacto") antes de terminar para no alterar el ambiente compartido — cualquier CP futuro que toque este modal debe hacer lo mismo.

### CP-312 — Opciones de orden en tabs + Crear recepción + Refrescar
El menú de opciones por tarjeta (`more_vert`, catálogo completo en sección 30) está disponible también en los tabs "Tablero" y "Repuestos", no solo en "Órdenes". El botón "+ Recepción" abre el mismo modal (`input[placeholder="Placa / Matricula"]`) desde cualquier tab — el CP lo abre y cierra sin crear ninguna orden. El botón `#btn_refresh_repair_order_list_cache` funciona en "Órdenes" y "Tablero", confirmado por cambio del texto "última actualización".

### CP-313 — Click en info cliente/vehículo → equivalente a "Ver orden" + Asignar mecánico
El área de placa/marca/modelo de cada tarjeta es `.reception-order-vehicle-line` (`onclick="getOrderDetailById(id)"`, el mismo mecanismo que "Ver orden" del kebab, sección 30) — clickearla agrega la clase `viewing-repair-order` al `<body>` y colapsa `#repair_order_search` a 0×0, igual que "Ver orden". Solo una recarga completa revierte el efecto (Escape no sirve, ya documentado en sección 30). **"Responsables" (asignar mecánico) solo está disponible en el tab "Órdenes"** — la vista compacta del tab "Tablero" no expone ese botón por tarjeta (hallazgo confirmado en vivo, no bloqueante).

### 🔴 Bloqueo confirmado — no se pudo cubrir Editar / Desactivar / Eliminar orden (items 11–13 del encargo)
Crear una orden nueva y desechable (necesaria para probar estas 3 acciones sin tocar una orden compartida) requiere completar el wizard hasta "Firma del cliente" y hacer clic en "Generar". **El pad de firma (`<canvas>`, sin librería identificada por nombre, clase `.drawpad-colorbox` para los colores) no es automatizable con Playwright**: se probaron 6+ técnicas (mouse moves/down/up con `steps` y delays realistas, dibujo directo vía Canvas 2D API `ctx.moveTo/lineTo/stroke`, eventos de puntero, selección de color previa obligatoria confirmada por el panel de instrucciones) — el canvas muestra contenido visualmente (confirmado con lectura de píxeles) pero la librería interna nunca lo registra como firma válida: el clic en "Generar" completa sin error pero ninguna orden nueva aparece luego en el buscador. Un intento adicional de usar la orden demo compartida ("Cliente Demo Defensa", id 12305, sección 30) para probar solo "Editar orden" (no destructivo, vía `getOrderById(id)` desde el menú `.options-menu-button`/`myDropdow<ID>` en tab "Órdenes" con buscador + Enter) quedó sin completar por un cuelgue del script de exploración en vivo — no se identificó la causa raíz. **Decisión del usuario**: cerrar este bloque documentando el bloqueo tal cual, sin forzarlo.

### Items 8–9 (Configurar Flujo de Trabajo / Permisos de Gestión de Taller / Configurar Pasos de la Recepción) — deliberadamente no ejercidos
Confirmado en vivo que estas 3 opciones sí existen, accesibles desde el mismo kebab del módulo (`#dLabel1299`) usado en CP-311: "Configurar Flujo de Trabajo" (habilita edición del flujo por etapa), "Permisos de Gestión de Taller" (permisos de usuarios del taller) y "Configurar Pasos de la Recepción" (activar/desactivar pasos del wizard). **No se ejercieron deliberadamente**: son ajustes a nivel de toda la cuenta/módulo, y durante esta sesión se confirmó que otras sesiones paralelas estaban creando órdenes activamente en este mismo ambiente compartido (ver nota de colisión abajo) — desactivar un paso del wizard o un permiso mientras otra sesión completa un wizard en curso podría romper su prueba. **Decisión del usuario**: saltarlos y documentar el riesgo, no intentarlos en este bloque.

### ⚠️ Nota de colisión de numeración (misma dinámica que sección 31)
Durante el cierre de este bloque se detectó que otra sesión paralela, compartiendo el mismo directorio físico sin worktrees aislados, tenía sin commitear un lote completo de CPs (CP-294 a CP-303: pasos del wizard "Nueva Recepción" — servicios, inspección, enderezado y pintura, abonos, partes del vehículo, fotos, marcación de daños, observaciones, firma, generar) que colisionaba numéricamente con CP-300–CP-303 de este bloque. CP-300 y CP-301 de este bloque ya estaban commiteados y pusheados a `origin/main` desde antes en esta misma sesión sin conflicto real (contenido idéntico/casi idéntico). Los archivos en conflicto real (CP-302 y CP-303 propios, sin commitear) se **renumeraron a CP-310 y CP-311** (y por consistencia también CP-304→CP-312, CP-305→CP-313), dejando intacto el lote de la otra sesión (CP-294–CP-303) sin tocarlo. Los 4 CPs renumerados se re-ejecutaron individualmente tras el cambio y confirmaron PASS antes de cerrar el bloque.

### Estado de implementación — ✅ cubierto dentro del alcance autorizado, con 2 bloqueos documentados (2026-08-19)
6 CPs (`tests-playwright/02-gestion-taller/01-recepcion-vehiculo/`): **CP-300**, **CP-301**, **CP-310**, **CP-311**, **CP-312**, **CP-313** — cubren 12 de los 17 ítems del encargo (1–7, 10, 14–17). Ítems 8–9 (config. de flujo/pasos/permisos) deliberadamente no ejercidos por riesgo en ambiente compartido concurrente. Ítems 11–13 (Editar/Desactivar/Eliminar orden) bloqueados por la limitación del pad de firma para crear una orden desechable — Desactivar/Eliminar además requieren confirmación explícita del usuario antes de ejecutarse sobre cualquier orden real, la cual no se ha solicitado en este bloque.

---

## 33. Recepción de Vehículo — Bloques 2-3, pasos del wizard "Nueva Recepción" (CP-291–CP-299, CP-304–CP-307, 2026-08-19/20)

Encargo con tiempo acotado ("quedan pocos días") explícitamente definido como **cobertura de muestra representativa, no exhaustiva**: 1-2 flujos centrales por cada paso del wizard de creación de una recepción (`/vehicularReception/vehicularQuickReception`, mismo wizard documentado en la sección 31), priorizando el recorrido más parecido a un uso real completo sobre la exhaustividad de cada bullet del encargo original. Reutiliza en todos los CPs un mismo patrón de arranque (placa → cliente existente "cliente prueba tarea 5" vía búsqueda real + clic en la tarjeta `h3.customer-name` → estilo "SEDAN" → marca id `131` fija + primer modelo real vía Chosen → N × clic en "Siguiente") para llegar al paso bajo prueba con el mínimo de pasos previos.

### Selectores y hallazgos confirmados por paso
- **Seleccionar Cliente (CP-291)**: seleccionar un cliente EXISTENTE desde el buscador (`#vehicular_reception_customer_search`) **avanza automáticamente** el wizard al siguiente paso — no hace falta clic en "Siguiente" aparte. El clic real debe apuntar al `h3.customer-name` cuyo texto coincide exactamente con el término buscado (`page.locator('h3.customer-name', { hasText: ... })`, no al primer `h3.customer-name` de todo el DOM — la página puede tener más de una tarjeta de cliente visible fuera del viewport actual, y tomar "la primera" a ciegas hace clic en la tarjeta equivocada sin lanzar ningún error). El modal de "Agregar Cliente nuevo" desde este wizard usa ids `cf_identifier`/`cf_name`/`cf_email`/`cf_address`/`cf_whatsapp`/`cf_phone_1` (**no** `c_identifier` como el modal equivalente del POS).
- **Seleccionar Estilo (CP-292)**: tarjetas reales `.card.style-vehicle`/`[onclick*="setVehicleStyle"]`; modal "Agregar Estilo" en `#vehicle_admin_style_name` + un `<select>` Chosen de tipo.
- **Detalles del vehículo (CP-293)**: el botón "+" junto a Marca y el acordeón "Aseguradora" no tienen ningún selector estable por texto/id/clase localizable vía recorrido del DOM — se resolvió con **clic por coordenadas fijas** (`page.mouse.click(830, 249)` y `(858, 939)` respectivamente), válido porque el viewport de este wizard es fijo (1440×1200) en todas las sesiones de este proyecto.
- **Seleccionar Servicios (CP-294)**: tarjetas de producto/servicio del catálogo se detectan de forma genérica por contener `₡` + `Und` (productos) excluyendo el texto "agregar producto", filtrando por bajo conteo de descendientes para descartar contenedores envolventes. **Riesgo de montos (sección 22)**: se extrajo el total del carrito (`extraerTotal`, regex `₡\s*([\d.,]+)`) y se marcó como corrupto cualquier total >₡100,000,000 para un carrito de 1 producto + 1 servicio — en las ejecuciones de este bloque el resultado fue `null` (no se disparó la corrupción), documentado como hallazgo negativo puntual, no como cierre del riesgo conocido.
- **Inspección (CP-295)**: se intentó ubicar un toggle/checkbox de "reemplazo de productos" cerca del texto `/reemplazo/i` — no se confirmó su existencia en esta ejecución (hallazgo no confirmado, no forzado más allá por criterio de muestra).
- **Enderezado y Pintura (CP-296)**: mismo patrón de detección genérica por `₡` para las tarjetas de las tabs "Servicios"/"Piezas".
- **Abonos (CP-297)**: `#initial-payment-repair-order` (monto), `#select_payed_with_ro` (forma de pago, Chosen), `#apply_to_cash_id` (caja, Chosen). Confirmado que el monto ingresado aparece reflejado en la tabla de historial de abonos tras "Guardar".
- **Partes del vehículo (CP-298)**: cada tarjeta expone 4 íconos de estado (buena/regular/mala/no tiene) y un botón "Agregar Parte". Ni el clic sobre el ícono de estado ni la reaparición de una parte recién creada en el listado se pudieron confirmar visualmente en esta ejecución (hallazgos no confirmados).
- **Seleccionar Fotos (CP-299)**: `input[type="file"]` oculto tras la zona de arrastrar-y-soltar, subido con `page.setInputFiles()` reutilizando una imagen real ya existente en `reports/screenshots/`. Confirmado con la aparición de un `<img>` con `src` de tipo blob/upload tras la subida.
- **Marcación de daños (CP-304)**: canvas de diagrama del vehículo, dibujado con una secuencia real `page.mouse.move/down/move/up` (no clics simples). Botón "Colocar texto" + clic sobre el canvas + `page.keyboard.type()` para el texto. "Guardar nueva" confirmado por texto exacto (regex `/guardar nueva/i`).
- **Observaciones generales (CP-305)**: `#damage_repair` (asesor de servicio) y `#damage_repair_message` (notas del cliente). Confirmado que ambos textos persisten (comparación estricta `===`) tras navegar "Siguiente" → "Anterior" y volver al paso.
- **Firma del cliente (CP-306)**: `<canvas>` de firma dibujado con la misma técnica de mouse real. Botón "Limpiar firma" confirmado funcional. Este CP deliberadamente **no** hace clic en "Generar" (lo cubre CP-307), para no duplicar la cobertura de generación de orden.
- **Navegación y Finalización (CP-307)**: navegación "Anterior"/"Siguiente" confirmada entre 3 pasos consecutivos (Firma → Observaciones → Marcación → Observaciones → Firma). El botón final "Generar" abre un **modal de confirmación propio del sistema, NO un SweetAlert2** (`¿Está seguro de generar la orden?` / botones "Cancelar" y "Generar orden", sin clases `swal*`) — buscar el botón de confirmación por su clase `.swal2-confirm` falla silenciosamente (el clic en "Generar" se ejecuta pero el modal de confirmación nunca se cierra); hace falta ubicarlo por texto exacto `/^generar orden$/i`. Confirmada la generación real de la orden (el wizard se cierra tras confirmar).

### ⚠️ Nota de numeración (misma dinámica que las secciones 31 y 32)
Al cerrar este bloque se detectó que otra sesión paralela, compartiendo el mismo directorio físico sin worktrees aislados, ya tenía **commiteados y pusheados a `origin/main`** `cp300-reporte-ordenes-admin-whatsapp.js` y `cp301-panel-recepcion-tabs-buscador.js` (sección 32) — contenido totalmente distinto al de los CP-300/CP-301 de este bloque (Marcación de daños / Observaciones generales), a diferencia de lo que el log de cierre de la sección 32 sugiere ("contenido idéntico/casi idéntico"). Para no pisar trabajo ya entregado y pusheado, los 4 CPs de este bloque que colisionaban (CP-300→**CP-304**, CP-301→**CP-305**, CP-302→**CP-306**, CP-303→**CP-307**) se renombraron — archivo y todas las referencias internas (nombre de función, placa de prueba, mensajes de consola) — y se **volvieron a ejecutar individualmente tras el cambio**, confirmando PASS los 4 antes de cerrar el bloque. CP-294–CP-299 no colisionaban con nada y se dejaron con su numeración original.

### Estado de implementación — ✅ cubierto como muestra representativa (2026-08-19/20)
13 CPs (`tests-playwright/02-gestion-taller/01-recepcion-vehiculo/`): **CP-291–CP-299**, **CP-304–CP-307** — un flujo central por paso del wizard desde "Seleccionar Cliente" hasta "Generar orden" (con "Seleccionar Cliente"/"Seleccionar Estilo"/"Detalles del vehículo" ya cubiertos previamente en este mismo rango de numeración). **Pendiente explícito para una sesión futura si se retoma este encargo** (por decisión de priorizar cobertura de muestra sobre exhaustividad, no por bloqueo técnico): un segundo flujo por paso donde el encargo original permitía elegir 1-2 (ej. estilo/marca/aseguradora nuevos en más profundidad), investigar a fondo los 3 hallazgos no confirmados (toggle de "reemplazo de productos" en Inspección, cambio de estado e indexación de parte nueva en Partes del vehículo), y una segunda pasada sobre el riesgo de montos (sección 22) con carritos más variados que el probado en CP-294.
